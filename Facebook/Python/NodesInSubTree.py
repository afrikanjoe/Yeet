...
def count_of_nodes(root, queries, s):
  ...
  if(not root):
    return output
  queue = [root]
  # we need to loop until we find the node 
  # we are looking for and bfs is a good start
  while queries: 
    query = queries.pop(0)
    queue = [root]
    while queue:
      node = queue.pop(0)
      val = (node.val,s[node.val-1])
      if(val==query):
        count = dfs_count(node,s)
        output.append(count)
        break
      else:
...
def dfs_count(node,s):
  count = 0 
  letter = s[node.val-1]
  queue = [node]
  while queue:
    new_node = queue.pop(0)
    new_letter = s[new_node.val-1]
