...
public class MinArrows {
    ...
    public static int findMinArrowShots(int[][] points) {
        ...
        // lamda function to sort by increasing endpoint
        Arrays.sort(points, (a, b) -> Integer.compare(a[1], b[1]));

        int overlapPoint = points[0][1];
        int count = 1;

        for (int i = 1; i < points.length; i++) {
            if (overlapPoint >= points[i][0]) {
                continue;
            }

            count++;
            overlapPoint = points[i][1];
    ...
    public static void main(String[] args) {
        ...
        int[][] example = { { 3, 16 }, { 2, 8 }, { 1, 6 }, { 7, 12 } };

        int[][] points = { { 1, 2 }, { 2, 3 }, { 3, 4 }, { 4, 5 } };

        Arrays.sort(example, (a, b) -> Integer.compare(a[1], b[1]));
